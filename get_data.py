#! /usr/bin/python
# Used to write an nmea.out file in the correct
# format for testing with gpsd-fake
# to get the correct format required by gpsd,
# which could then be used for the send_data script
#
# This sends data out on port 49003
# (should be constant rather than magic number) in a
...
def write_out():
    ...
    str_time = "%i%i%i" % (cur_time_struct[3], cur_time_struct[4],
                           cur_time_struct[5])
    params_dict["time"] = str_time
    str_date = "%i%i%i" % (cur_time_struct[0], cur_time_struct[1],
                           cur_time_struct[2])
...
def record_results(data_list):
    ...
    if data_type == 3:
        params_dict["ktas"] = str("%3.2f" %
                                  float(convert_to_float(data_list[3])))
    elif data_type == 19:
        params_dict["mag_head"] = str("%3.2f" %
                                      float(convert_to_float(data_list[1])))
        params_dict["mag_var"] = str("%3.2f" %
                                     float(convert_to_float(data_list[2])))
    elif data_type == 20:
        ...
        params_dict["alt_agl"] = str("%8.2s" %
                                     convert_to_float(data_list[4]))
        params_dict["lat_north"] = str("%3.7s" %
                                       convert_to_float(data_list[5]))
        params_dict["lon_west"] = str("%3.7f" %
                                      convert_to_float(data_list[6]))
...
def process_data(data_byte_list):
    ...
    # data_bytes = struct.unpack(str(
    # len(data_byte_list)) + "b", data_byte_list)
...
def main():
    ...
    while 1:
        ...
        # print len(data)
        # print struct.unpack("lccccccccccccccc
        # cccccccccccccccccc", data)
        # print struct.unpack("bbbbbbbbbbbbbbbb
        # bbbbbbbbbbbbbbbbbbbbbbbbb", data)
        ...
        # break
        # runs += 1
        # other_data = struct.unpack("ccccccccccccccccccc
        # cccccccccccccccccccccc", data)
        # break
    s.close()
    # print params_dict
    # formats = ["x", "c", "b","B", "?", "h", "H", "i",
    # "l", "L", "q", "Q", "f", "d", "s","p","P"]
    # for i in formats:
    # try:
    # x = ['Y', '\xdb', '!', 'B']
    # print struct.unpack('<f', struct.pack('4b', *x))
    # bob= struct.unpack("bbbbbbbbbbbbbbbbbbbb
    # bbbbbbbbbbbbbbbbbbbbb", tmp_data)
    # bob= struct.unpack("bbbbbbbbbbbbbbbbbbbbbbbbbbbb
    # bbbbbbbbbbbbb", tmp_data)
    # print "bob: ", bob
    # print "bob2: ", struct.pack('>b', *bob)
    # print struct.unpack("bbbbb" + "bbbb" + "d" + "bbbbbb
    # bbbbbbbbbbb", tmp_data)
    # get_results(struct.unpack("bbbbbbbbbbbbbbbbbbbbb
    # bbbbbbbbbbbbbbbbbbbb", tmp_data))
    # except:
    # pass
    # x = [19, 0, 0, 0]
    # print struct.unpack('<f', struct.pack('4b', *x))
...
'''
_Vind,_kias |   _Vind,_keas |   Vtrue,_ktas |   Vtrue,_ktgs |
   _Vind,__mph |   Vtrue,mphas |   Vtrue,mphgs |   __lat,__deg |
      __lon,__deg |   __alt,ftmsl |   __alt,ftagl |   ___on,runwy |
